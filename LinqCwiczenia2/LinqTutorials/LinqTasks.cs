...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.Where(x => x.Job == "Frontend programmer" && x.Salary > 1000).OrderByDescending(x => x.Ename);
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Join(Depts, x => x.Deptno, y => y.Deptno, (x, y) => new { x.Ename, x.Job, y.Dname });
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.GroupBy(x => x.Job).Select(x => new { Praca = x.Key, LiczbaPracownikow = x.Count() });
            return result;
        }
        ...
        {
            Emp result = Emps.Where(x => x.Job == "Frontend programmer").OrderByDescending(x => x.HireDate).FirstOrDefault();
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Select(x => new { x.Ename, x.Job, x.HireDate }).Union(new[] { new { Ename = "Brak wartości", Job = (string)null, HireDate = (DateTime?)null } });
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.GroupBy(x => x.Deptno).Where(x => x.Count() > 1).Select(x => new { name = x.Key, numOfEmployees = x.Count() });
            return result;
        }
        ...
        {
            int result = arr.GroupBy(x => x).Where(x => x.Count() % 2 != 0).Select(x => x.Key).FirstOrDefault();
            //result=
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            ...
            IEnumerable<Dept> result = Depts.GroupJoin(Emps, x => x.Deptno, y => y.Deptno, (x, y) => new { x, y }).Where(x => x.y.Count() == 5 || x.y.Count() == 0).Select(x => x.x).OrderBy(x => x.Dname);
            //result =
            return result;
        }
    }
